...
-- Based on AwesomeWM's awful.client with foundation.object integration
...
-- Create a client object wrapper with foundation.object features
local function create_client_object(c_client)
  ...
  -- Create new client object based on foundation.object
  ...
  obj:add_property("geometry", {
    getter = function(self)
      local c = self:get_private().c_client
      return c and Some.client_get_geometry(c) or foundation.geometry.rectangle(0, 0, 0, 0)
    end,
  ...
  function obj:move(x, y)
    ...
    self.geometry = foundation.geometry.rectangle(x, y, geom.width, geom.height)
  end
  
  function obj:resize(w, h)
    local geom = self.geometry
    self.geometry = foundation.geometry.rectangle(geom.x, geom.y, w, h)
  end
...
function client.connect_signal(signal_name, callback)
  foundation.signal.connect("client::" .. signal_name, callback)
end

function client.disconnect_signal(signal_name, callback)
  foundation.signal.disconnect("client::" .. signal_name, callback)
end
...
-- Advanced client operations using foundation.geometry
function client.arrange_tiled(clients, area, master_count)
  master_count = master_count or 1
  
  if #clients == 0 then return end
  
  -- Use foundation.geometry for layout calculations
  if #clients == 1 then
  ...
  if master_count >= #clients then
    ...
    local areas = foundation.geometry.split_horizontal(area, #clients)
    for i, c in ipairs(clients) do
      c.geometry = areas[i]
    end
  else
    ...
    if master_count > 0 then
      master_area = foundation.geometry.rectangle(area.x, area.y, area.width / 2, area.height)
      stack_area = foundation.geometry.rectangle(area.x + area.width / 2, area.y, area.width / 2, area.height)
